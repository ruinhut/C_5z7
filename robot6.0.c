...
int robotTurn(int player , int board[SIZE][SIZE] , struct singleScore robotScoreBoard[SIZE][SIZE] , struct singleScore enemyScoreBoard[SIZE][SIZE]){
                ...
                for(j = 0; j < 3 ; j++){
                    if(board[enemytop3[j][1]][enemytop3[j][2]] == 0){
                        ...
                        if(tempScore < 0 && -tempScore > maxScore[i]){
                            ...
                            //如果我方-敌方总分更高也就是我方总分相对高，敌方相对低，则更新落子点
    ...
    printf("%d %d %d\n",mytop3[0][1],mytop3[0][2],mytop3[0][0]);
    printf("%d %d %d\n",mytop3[1][1],mytop3[1][2],mytop3[1][0]);
    printf("%d %d %d\n",mytop3[2][1],mytop3[2][2],mytop3[2][0]);
...
//5.人类回合函数
int humanTurn(int player , int board[SIZE][SIZE]){
...
int main(){
    ...
    arrayForInnerBoardLayout[2][6] = 1;
    for(i=0;i<SIZE;i++){
        for(j = 0; j < SIZE ; j++){
    ...
    printf("禁手吗：%d\n",forbidden(1,2,5,arrayForInnerBoardLayout));
